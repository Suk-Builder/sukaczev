  ...
  try {
    ...
    await channel.assertQueue(config.rabbitmq.queue, {
      ...
      arguments: {
        ...
        'x-dead-letter-routing-key': `${config.rabbitmq.queue}.dlq`,
      },
    });
    await channel.assertQueue(`${config.rabbitmq.queue}.dlq`, { durable: true });
    await channel.bindQueue(config.rabbitmq.queue, config.rabbitmq.exchange, 'video.*');
